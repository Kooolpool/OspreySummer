...
import static org.firstinspires.ftc.osprey.subsystems.BarcodePipeline.BarcodeConstants.*;
...
public class BarcodePipeline extends OpenCvPipeline implements Supplier<Integer> {
            ...
            if (DISPLAY) {
                Imgproc.rectangle(
                    ...
                    new Rect(
                        BORDER_LEFT_X,
                        BORDER_TOP_Y,
                        input.width() - BORDER_RIGHT_X - BORDER_LEFT_X,
                        input.height() - BORDER_BOTTOM_Y - BORDER_TOP_Y
                    ),
                    DISPLAY_COLOR,
                    2
                );
    ...
    public synchronized Integer get() {
        if (getRectArea() > MIN_AREA) {
            ...
            if (Math.abs(p.x - CENTER.x) < VARIANCE && Math.abs(p.y - CENTER.y) < VARIANCE) last =
                1;
